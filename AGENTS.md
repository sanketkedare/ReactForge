...
4. **Browser Subagent Policy**: Do NOT open the browser subagent for visual verification on routine turns unless explicitly requested by the user. Rely on `npx tsc --noEmit` and file checks for routine validation.
...
8. **Build Gate — EXPLICIT REQUEST ONLY**: NEVER run `npm run build`, `next build`, or any production build command unless the user explicitly asks for it in that turn. Type-checking with `npx tsc --noEmit` is the only automated validation allowed. Running a full build on every code change is wasteful and forbidden.
9. **Git Gate — EXPLICIT REQUEST ONLY**: NEVER run `git commit`, `git push`, `git add`, or any git write command unless the user explicitly asks for a commit in that turn. Do NOT auto-commit after completing a task, after a build passes, or as a "cleanup" step. Generating a commit message suggestion in chat is allowed — actually executing `git commit` is NOT unless instructed.
